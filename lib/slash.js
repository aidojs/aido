const { isArray, isString, each, set, forIn, isObject } = require('lodash')
...
class Slash {
  ...
  async getPersistedSession() {
    ...
    return this.database.Session.query()
      .first()
      .where(function() {
        this.whereNull('team').where('user', slackId) // Get older sessions which don't have a team
      })
      .orWhere(function() {
        this.where('team', team).where('user', slackId)
      })
  }
